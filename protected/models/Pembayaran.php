...
class Pembayaran extends CActiveRecord
{
    ...
    public function fetch()
    {
        $query = "
            ...
            JOIN pembayaran
                ON pembayaran.pendaftaran_id = pendaftaran.pendaftaran_id
            ORDER BY pendaftaran.created_date DESC
        ";
        return Yii::app()->db->createCommand($query)->queryAll();
    }

    public function getHargaObatById($id)
    {
        ...
        $query = "
            SELECT  pendaftaran.*
                ,   obat.obat_nama
                ,   obat.obat_harga
                ,   tindakanobat_detail.obat_jumlah
            FROM pendaftaran
            JOIN tindakanobat_detail
                ON tindakanobat_detail.pendaftaran_id = pendaftaran.pendaftaran_id
            JOIN obat
    ...
    {
        $pembayaran_id      = htmlspecialchars($_POST['pembayaran_id'], ENT_QUOTES);
        $pendaftaran_id     = htmlspecialchars($_POST['pendaftaran_id'], ENT_QUOTES);
        $pembayaran_pasien  = htmlspecialchars($_POST['pembayaran_pasien'], ENT_QUOTES);
        $pembayaran_kembali = htmlspecialchars($_POST['pembayaran_kembali'], ENT_QUOTES);
        
        $query = "
            START TRANSACTION;
                UPDATE pembayaran SET 
                    pembayaran_pasien   = $pembayaran_pasien,
                    pembayaran_kembali  = $pembayaran_kembali,
                    pembayaran_status   = 1
                WHERE pembayaran_id = $pembayaran_id;

                UPDATE pendaftaran SET 
                    pendaftaran_status = '3'
	...
	// {
	// 	return array(
	// 		'pendaftaran_id' => 'Pembayaran',
	// 		'pendaftaran_nama' => 'Pembayaran Nama',
	// 		'pendaftaran_tempatLahir' => 'Pembayaran Tempat Lahir',
	// 		'pendaftaran_tglLahir' => 'Pembayaran Tgl Lahir',
	// 		'pendaftaran_alamat' => 'Pembayaran Alamat',
